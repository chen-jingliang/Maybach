...
const mkQ=(cap,mx,nx=CFG.upNMax)=>{const k=mkK(cap);let n=0;return{get empty(){return k.e()},get b(){return k.b},clear(){k.clear();n=0},sow:d=>{const z=d?.byteLength||0;if(!z||k.b+z>mx||n>=nx)return 0;n++;return k.sow(d)},bundle:d=>{const r=k.pack(d);n=Math.max(0,n-r[2]);return r}}};
...
async function f26_pool(h, p, pool) {
    try {
        const sock = await f_cd(h, p, 2500); 
        return sock;
    } catch {}
    
    const via = async (q) => "socks5" === q.type ? f20(q, h, p) : ["http", "https"].includes(q.type) ? f21(q, h, p) : "sstp" === q.type ? f22(q, h, p) : ["turn", "turns"].includes(q.type) ? f38(q, h, p) : f_cd(q.host, q.port, 2500);
...
export default {
    async fetch(req, env) {
        ...
        // 【安全补丁】：加入碎包引流缓冲机制，杜绝 XHTTP 环境下解析判定为假而断流的问题
