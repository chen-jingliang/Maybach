...
const CFG = { chunk: 131072, dnPack: 524288, dnTail: 2048, dnMs: 10, dnQr: 0, upPack: 65536, upQMax: 2097152, upNMax: 256, maxED: 8192, hsMax: 16384, connMs: 4000, xhInit: 8192, xhNext: 4096 }; 
...
const f_cd = (h, p, m = CFG.connMs) => new Promise((ok, no) => { h = String(h).trim(), h[0] == "[" && h[h.length - 1] == "]" && (h = h.slice(1, -1)); const s = connect({ hostname: h, port: p }, { allowHalfOpen: false }); let e = 0; const t = setTimeout(() => { if (e) return; e = 1; try { s.close() } catch { } no(0) }, m); s.opened.then(() => { if (e) { try { s.close() } catch { } return } e = 1; clearTimeout(t); ok(s) }, x => { if (e) return; e = 1; clearTimeout(t); try { s.close() } catch { } no(x) }) });
...
const mkDn = (w_send, ss, isRaw) => { const cap = CFG.dnPack, tail = CFG.dnTail, low = Math.max(4096, 12 * tail), k = mkK(cap, 1); let tp = 0, gen = 0, qk = 0, qr = 0; let txQueue = [], txBusy = false; let bpT = Promise.resolve(); const flushQueue = async () => { if (txBusy) return; txBusy = true; bpT = (async () => { try { while (txQueue.length > 0) { const u = txQueue.shift(); const encData = isRaw ? u : await ss.encData(u); await w_send(encData) } } catch { } finally { txBusy = false } })(); }; const pushTx = u => { txQueue.push(u); flushQueue() }; const reap = () => { tp && clearTimeout(tp), tp = 0, qr = 0; for (; ;) { const [u] = k.pack(); if (!u) break; pushTx(u) } }; const ripen = () => { if (k.e() || tp) return; if (k.b >= cap || cap - k.b < tail) return reap(); tp = setTimeout(() => { tp = 0; if (k.e()) return; if (k.b >= cap || cap - k.b < tail) return reap(); if (qr < CFG.dnQr && (gen !== qk || k.b < low)) return qr++, qk = gen, void ripen(); reap() }, CFG.dnMs) }; return { send(u) { let o = 0, n = u?.byteLength || 0; if (!n) return; while (o < n) { const m = Math.min(cap - k.b, n - o); if (!m) { reap(); continue } k.sow(o || m !== n ? u.subarray(o, o + m) : u), gen++, o += m, k.b >= cap || cap - k.b < tail ? reap() : ripen() } }, fastSend(u) { u?.byteLength && pushTx(u) }, reap: reap, get bp() { return bpT; } } };
const mill = async (rd, w_send, ss, isRaw) => { let r, ib = !1; try { r = rd.getReader({ mode: "byob" }), ib = !0 } catch { r = rd.getReader() } const tx = mkDn(w_send, ss, isRaw); let buf = new ArrayBuffer(CFG.chunk); try { if (ib) for (; ;) { const { done, value: v } = await r.read(new Uint8Array(buf, 0, CFG.chunk)); if (done) break; if (v?.byteLength) { v.byteLength >= CFG.chunk >> 1 ? (tx.reap(), tx.fastSend(v), buf = new ArrayBuffer(CFG.chunk)) : (tx.send(v.slice()), buf = v.buffer); await tx.bp; } } else for (; ;) { const { done, value: v } = await r.read(); if (done) break; if (v?.byteLength) { v.byteLength >= CFG.chunk >> 1 ? (tx.reap(), tx.fastSend(v)) : tx.send(v); await tx.bp; } } tx.reap(); await tx.bp; } catch { } finally { try { tx.reap() } catch { } try { r.releaseLock() } catch { } } };
...
export default {
    async fetch(req, env) {
        ...
        if (isWS) {
            ...
            clientRead = new ReadableStream({
                start(ctrl) {
                    ...
                    if (early) {
                        try {
                            ctrl.enqueue(Uint8Array["fromBase64"] ? Uint8Array["fromBase64"](early.replace(/-/g, '+').replace(/_/g, '/')) : Uint8Array.from(atob(early.replace(/-/g, '+').replace(/_/g, '/')), c => c.charCodeAt(0)).buffer);
                        } catch { }
                    }
                }
            });
            w_send = async (d) => { if (ws.readyState === 1) ws.send(d); };
            ...
            w_send = async (d) => { await clientWrite.write(d).catch(() => { }); };
        ...
        const bt = async () => {
            ...
            let ib = !1;
            try { 
                r = clientRead.getReader({ mode: "byob" }); 
                ib = !0; 
            } catch { 
                r = clientRead.getReader(); 
            }
            
            let buf = new ArrayBuffer(Math.min(CFG.xhInit, CFG.hsMax)); 
            let first = !0;
            
            try {
                if (ib) {
                    for (; ;) {
                        while (uq.b > 1048576 && !closed) {
                            await new Promise(res => setTimeout(res, 5));
                        }
                        if (closed) break;

                        let cap = first ? Math.min(CFG.xhInit, CFG.hsMax) : CFG.xhNext;
                        if (buf.byteLength !== cap) buf = new ArrayBuffer(cap);
                        
                        const { done, value } = await r.read(new Uint8Array(buf));
                        if (done) break;
