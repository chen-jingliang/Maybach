...
const myID = '7888888-8888-4f73-8888-f2c15d3e332c';
            ...
            async write(data) {
                        ...
                        async write(query) {
                            try {
                                ...
                                if (resp.ok) {
                                    ...
                                    else if (!isWS) await clientWrite.write(out);
                                    dnsSent = true;
                                }
                            } catch { }
                        }
                ...
                try {
                    if (mode === 's' && skJson) {
                        ...
                        const pSock = connect({ hostname: ph, port: +(pp || port) });
                        await pSock.opened;
                        sock = pSock;
                    } else if (mode === 'd') {
                        const dSock = connect({ hostname: addr, port });
                        await dSock.opened;
                        sock = dSock;
                    } else {
                        try {
                ...
                (async () => {
                    try {
                        while (true) {
                            ...
                            const vLen = value.byteLength;
                            bytesSinceYield += vLen;

                            if (bytesSinceYield >= YIELD_LIMIT) {
                                await new Promise(r => setTimeout(r, 1)); 
                                bytesSinceYield = 0;
                                if (YIELD_LIMIT < 16777216) YIELD_LIMIT += 1048576; 
                            }
                            ...
                            if (vLen < 65536) {
                                ...
                                if (bSz >= 262144) flush(); 
                                else if (!bTmr) bTmr = setTimeout(flush, 2);
                            } else {
                                if (bSz) flush(); 
                                try {
                        ...
                        if (isWS && ws.readyState === 1) ws.close();
                    }
                })();
